...
<body class="bg-[url(/img_bg/bg-1.jpg)] bg-cover">
    <div class="flex flex-col justify-center items-center min-h-screen">
        <div class="backdrop-blur-lg bg-white/10 border border-white/20 p-5 rounded-lg shadow-lg">
            <h2 class="text-2xl font-bold p-4 drop-shadow-md text-white text-center">Edit Jabatan Pegawai</h2>
            <form action="{{ route('positions.update', $position->id) }}" method="POST">
                @csrf
                @method('PUT')
                <table>
                    <tr>
                        <td><label for="nama_jabatan" class="text-white">Nama Jabatan</label></td>
                        <td><input type="text" name="nama_jabatan"
                                value="{{ old('nama_jabatan', $position->nama_jabatan) }}" class="ml-5 rounded-lg text-sm backdrop-blur-sm bg-white/15 border border-white/20 text-white"></td>
                    </tr>
                    <tr>
                        <td><label for="gaji_pokok" class="text-white">Gaji Pokok Jataban</label></td>
                        <td><input type="text" name="gaji_pokok" value="{{ old('gaji_pokok', $position->gaji_pokok) }}"
